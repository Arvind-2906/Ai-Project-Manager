...
import { getSession, isInternalServiceRequest } from "@/lib/auth/session";
import {
  AuthenticationError,
  AuthorizationError,
  NotFoundError,
 ...
 * OR originates from an authorized internal service (FastAPI) presenting x-internal-secret.
 *
 * @param {Request} request Next.js incoming request
 * @returns {Promise<{ user: Object, session: Object, isInternal?: boolean }>}
 * @throws {AuthenticationError} If session is missing or expired
 */
export async function requireAuth(request) {
  if (request && isInternalServiceRequest(request)) {
    return {
      user: {
        id: null,
        name: "AI Agent Swarm",
        email: "ai-agent@internal.system",
        role: "AGENT",
      },
      session: {
        id: "internal-agent-session",
        userId: null,
      },
      isInternal: true,
    };
...
/**
 * 4. requireProjectMember(userId, projectId, isInternal)
 ...
 * If the request is from an authorized internal service (isInternal or userId === null),
 * verifies project existence and permits system operations.
 *
 ...
 */
export async function requireProjectMember(userId, projectId, isInternal = false) {
  if (!projectId) {
    throw new AuthorizationError("Project ID is required.");
  }

  // Internal AI service caller verification
  if (isInternal || userId === null) {
    ...
    return { project, projectMember: null, orgMember: null, isInternal: true };
  }
...
export async function requireProjectRole(userId, projectId, allowedRoles = []) {
  if (!userId) {
    throw new AuthorizationError("User authentication is required for role-protected actions.");
  }
